    ...
    for (const event of events) {
        if (event.status.type.description !== "Postponed") {
            ...
            if (eventDate >= today || event.status.type.state === "in" || event.status.type.description === "Halftime" || event.status.type.description === "Full Time") {
                ...
                const match_URL = `https://club.sportsurge.uno/#${homeTeam.team.shortDisplayName} vs ${awayTeam.team.shortDisplayName}`;
  ...
  for (const event of events) {
    ...
    const circuitfullname = event.circuit.fullName;
      ...
      for (const competition of competitions) {
        ...
        const formula_URL = `https://club.sportsurge.uno#${nameofevent}`;
...
async function getmmafixture() {
  ...
  for (const event of events) {
      if (event.status.type.description !== "Postponed"){
        ...
        console.log(events);
  const mma_URL = `https://club.sportsurge.uno/#${fightnight}`;
  if (event.status.type.state === "pre" ){
...
async function processGolfData(data, containerId) {
  ...
  for (const event of events) {
    if (event.status.type.description !== "Postponed") {
      ...
      const golfurl = `https://club.sportsurge.uno/#${eventname}`;
      const container = document.querySelector(`#${containerId}`);
      const teamContainer = document.createElement('div');
...
async function getindy() {
  ...
  for (const event of events) {
      if (event.status.type.description !== "Postponed"){
        ...
        console.log(events);
  const indyurl = `https://club.sportsurge.uno/#${indyrace}`;
  if (event.status.type.state === "pre" ){
